...
CHIP_ERROR err =CHIP_NO_ERROR;
    ...
    case Clusters::OnOff::Commands::On::Id:
        Clusters::OnOff::Commands::On::Type onCommand;
         err=Controller::InvokeGroupCommandRequest(&exchangeMgr, binding.fabricIndex, binding.groupId, onCommand);
        if (err != CHIP_NO_ERROR)
        {
                    ChipLogError(NotSpecified, "OnOff group command failed: %" CHIP_ERROR_FORMAT, err.Format());
...
CHIP_ERROR BindingGroupBindCommandHandler(int argc, char ** argv)
{
    ...
    entry->fabricIndex             = static_cast<chip::FabricIndex>(atoi(argv[0]));
    entry->groupId                 = static_cast<chip::GroupId>(atoi(argv[1]));
...
CHIP_ERROR BindingUnicastBindCommandHandler(int argc, char ** argv)
{
    ...
    entry->fabricIndex             = static_cast<chip::FabricIndex>(atoi(argv[0]));
    entry->nodeId                  = static_cast<chip::NodeId>(strtoull(argv[1], nullptr, 10));
    entry->local                   = 1; // Hardcoded to endpoint 1 for now
    entry->remote                  = static_cast<chip::EndpointId>(atoi(argv[2]));
